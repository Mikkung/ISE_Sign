...
begin
  ...
  if coalesce(p_intent, 'save') <> 'restore_default'
    and exists (select 1 from public.workflow_steps where project_id = p_project_id)
    and not exists (select 1 from pg_temp.student_workflow_input where step_id is not null)
  then
    raise exception 'The workflow could not be saved. Reload the latest workflow and try again.';
  end if;

...
begin
  ...
  select
    ...
    left(nullif(btrim(coalesce(step ->> 'name', '')), ''), 160),
  ...
  update pg_temp.student_workflow_input
    set step_name = case when step_name is null then
      case when role_type = 'staff' then 'Staff Review' else 'Faculty Approval' end
    else step_name end;

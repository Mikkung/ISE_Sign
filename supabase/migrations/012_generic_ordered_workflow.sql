alter table public.workflow_steps
  add column if not exists assignee_role public.app_role,
  add column if not exists assignment_mode text not null default 'specific_users';
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'workflow_steps_assignee_role_allowed'
      and conrelid = 'public.workflow_steps'::regclass
  ) then
    alter table public.workflow_steps
      add constraint workflow_steps_assignee_role_allowed
      check (assignee_role in ('staff', 'approver', 'admin'));
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'workflow_steps_assignment_mode_allowed'
      and conrelid = 'public.workflow_steps'::regclass
  ) then
    alter table public.workflow_steps
      add constraint workflow_steps_assignment_mode_allowed
      check (assignment_mode in ('any_active_role', 'specific_users'));
  end if;
end $$;
...
set assignee_role = coalesce(
  (
    ...
    join public.profiles profile on profile.id = assignment.approver_id
    where assignment.workflow_step_id = step.id
      and profile.role in ('staff', 'approver', 'admin')
    group by profile.role
    order by count(*) desc, profile.role::text
    limit 1
  ),
  case when step.certification_required then 'approver'::public.app_role else 'staff'::public.app_role end
)
where step.assignee_role is null;

with step_counts as (
  select
    step.id,
    step.assignee_role,
    count(distinct assignment.approver_id) filter (where assignment.invalidated_at is null) as assignment_count
  from public.workflow_steps step
  left join public.workflow_assignments assignment on assignment.workflow_step_id = step.id
  group by step.id, step.assignee_role
),
active_counts as (
  select role as assignee_role, count(*) as active_count
...
set assignment_mode = case
  when coalesce(step_counts.assignment_count, 0) > 0
    and step_counts.assignment_count = coalesce(active_counts.active_count, 0)
    then 'any_active_role'
  else 'specific_users'
end
from step_counts
left join active_counts on active_counts.assignee_role = step_counts.assignee_role
where step.id = step_counts.id;

alter table public.workflow_steps
  alter column assignee_role set not null;

create index if not exists workflow_steps_project_assignee_role_idx
  on public.workflow_steps(project_id, assignee_role, step_order);
...
begin
  ...
  if jsonb_typeof(p_steps) <> 'array' or jsonb_array_length(p_steps) < 1 then
  ...
  select
    ...
    left(btrim(coalesce(step ->> 'name', '')), 160),
  ...
  if exists (select 1 from pg_temp.student_workflow_input where step_name = '') then
    raise exception 'Each workflow step needs a valid non-empty name.';
  end if;
  ...
  select
    coalesce(input.step_id, gen_random_uuid()),
    p_project_id,
    input.step_name,
    input.final_order,
  ...
  select step.step_order, step.id
  from public.workflow_steps step
  where step.project_id = p_project_id;
  ...
  select
    ...
    case
      when v_project.status::text in ('submitted', 'approval_pending', 'revision_required') and step_map.final_order = 1 then 'waiting'
      else 'waiting'
    end,
    input.due_at,
    v_user_id
